<div>
    ...
    <div class="mb-6">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 space-y-4 sm:space-y-0">
            ...
            <h1 class="text-2xl font-bold text-gray-900">Donation #{{ $donation->id }}</h1>
                    <p class="text-sm text-gray-500">{{ $donation->type_label }} • {{ $donation->created_at->format('M j, Y') }}</p>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
                        ...
                        <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                            ...
                            @if(isset($donation->details['estimated_value']))
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">
                                        @if(is_array($donation->details['estimated_value']))
                                            ${{ number_format(array_sum($donation->details['estimated_value']), 2) }}
                                        @else
                                            ${{ number_format($donation->details['estimated_value'], 2) }}
                                        @endif
                                    </dd>
                                </div>
                            @endif
                    ...
                    <div class="px-6 py-4">
                        <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <dd class="mt-1 text-sm text-gray-900">
                                    @if(isset($donation->details['currency']))
                                        @if(is_array($donation->details['currency']))
                                            {{ implode(', ', $donation->details['currency']) }}
                                        @else
                                ...
                                <dd class="mt-1 text-sm text-gray-900">
                                    @if(isset($donation->details['payment_method']))
                                        @if(is_array($donation->details['payment_method']))
                                            {{ implode(', ', $donation->details['payment_method']) }}
                                        @else
                                            {{ $donation->details['payment_method'] }}
                                        @endif
                                    @else
                                        N/A
                                    @endif
            ...
            <div class="bg-white shadow rounded-lg">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h2 class="text-lg font-medium text-gray-900">Donor Information</h2>
                </div>
                <div class="px-6 py-4">
                    <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Name</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $donation->donor->first_name }} {{ $donation->donor->last_name }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Email</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $donation->donor->email }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Phone</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $donation->donor->phone }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
        ...
        <div class="space-y-6">
            ...
            <!-- Add Remark -->
            <x-add-remark :canModify="true" />

        <!-- Status Update Card -->
        <x-status-update :donation="$donation" :accounts="$accounts" :canModify="true" :newStatus="$newStatus" :statusOptions="$statusOptions" />

        <!-- Assignment Card -->
        <x-assignment-info :donation="$donation" :canModify="true" :volunteers="$volunteers" :selectedVolunteerId="$selectedVolunteerId ?? ''" :assignmentNote="$assignmentNote ?? ''" />
        </div>
    </div>

    <!-- Remarks History -->
    <div class="mt-6">
        <x-remarks-history :donation="$donation" :canViewRemarks="true" />
    </div>
