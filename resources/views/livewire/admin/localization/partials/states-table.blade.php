                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($states as $state)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $state->code ?: '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $state->country->name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                    {{ $state->volunteer_assignments_count }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($state->is_active)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    <button wire:click="openModal({{ $state->id }})" 
                                            class="text-indigo-600 hover:text-indigo-900">Edit</button>
                                    <button wire:click="toggleStatus({{ $state->id }})" 
                                            class="text-yellow-600 hover:text-yellow-900">
                                        {{ $state->is_active ? 'Deactivate' : 'Activate' }}
                                    </button>
                                    <button wire:click="delete({{ $state->id }})" 
                                            wire:confirm="Are you sure you want to delete this state? This action cannot be undone."
